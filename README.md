...
本项目后端无公网部署地址，完整本地启动+全功能操作演示录屏存放于仓库 `/video/demo_record.mp4`（百度网盘备份：链接: `https://pan.baidu.com/s/1EtnEYvppp9mqwq64Sk48Lw?pwd=m2ta` 提取码: m2ta），用于验证全部页面、接口功能正常运行。

## 线上体验入口

 微信小程序体验二维码：

微信小程序体验二维码（有效期至7月26日）：
![小程序体验二维码](./screenshot/ai/mini_qrcode.png)
扫描上方二维码可直接打开体验版小程序，首页路径 pages/index/index
小程序AppID：wx39261368150bcef2
...
1. 克隆仓库
   ```bash
  git clone https://github.com/wzy038/coffee-shop-mini.git
   ```
...
接口完整测试示例、成功/异常返回样例全部记录于根目录`API.md`标准化接口文档，本次以规范文档替代Postman测试截图，可结合本地演示录屏完整验证接口运行效果；
...
### 本地完整功能演示录屏
后端无公网部署，本录屏作为线上Demo兜底验证材料。
文件路径：./video/demo_record.mp4
